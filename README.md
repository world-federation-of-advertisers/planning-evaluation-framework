...
subdirectories.  Each of these subdirectories represents a Dataset.  An evaluation is performed
by running every model configuration against every Dataset.

An experiment consists of evaluating a collection of different models and parameters against
a collection of Datasets.  Several example experiments can be found in the `driver`
subdirectory:
...
*   `analysis`: notebooks that generate interpretable results from the data
