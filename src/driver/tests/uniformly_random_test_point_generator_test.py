...
from wfa_planning_evaluation_framework.driver.uniformly_random_test_point_generator import UniformlyRandomTestPointGenerator
...
class DataSetTest(absltest.TestCase):
    ...
    def test_one_publisher(self):
        ...
        generator = UniformlyRandomTestPointGenerator(data_set, np.random.default_rng(1))
    ...
    def test_fifty_publishers(self):
        pdf_list = []
        for i in range(50):
            pdf = PublisherData([(1, 0.01), (2, 0.02), (1, 0.04), (3, 0.05)], "pdf{}".format(i))
            pdf_list.append(pdf)
