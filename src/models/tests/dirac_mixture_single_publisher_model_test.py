...
class DiracMixtureSinglePublisherModelTest(absltest.TestCase):
    ...
    def test_obtain_zero_included_histogram(self):
        ...
        res = self.cls.obtain_zero_included_histogram(universe_size=5, rp=rp)
        expected = np.array([1, 0])
        self.assertSequenceAlmostEqual(res, expected, msg="Unexpected for zero reach")
        ...
        self.assertSequenceAlmostEqual(
            res, expected, msg="Unexpected for non-zero reach"
        )
    ...
    def test_fit(self):
        ...
        expected = np.array([1, 0])
        self.assertSequenceAlmostEqual(
            model.optimizer.ws, expected, places=2, msg="Unexpected for zero reach"
        )
        ...
        self.assertSequenceAlmostEqual(
            model.optimizer.ws, expected, places=2, msg="Unexpected for non-zero reach"
        )

    def test_by_impressions(self):
        rp = ReachPoint(impressions=[10], kplus_reaches=[3])
        model = self.cls(data=[rp], ncomponents=2)
        res = model.by_impressions(impressions=[0])
        expected = 0
        self.assertEqual(
            res._kplus_reaches[0],
            expected,
            msg="Unexpected for zero impression",
        )
        res = model.by_impressions(impressions=[10])
        expected = 3
        self.assertEqual(
            res._kplus_reaches[0],
            expected,
            msg="Unexpected for same impressions",
        )
        res = model.by_impressions(impressions=[20])
        expected = 3
        self.assertEqual(
            res._kplus_reaches[0],
            expected,
            msg="Unexpected for double impressions",
        )
        ...
        self.assertEqual(
            res._kplus_reaches[0],
            expected,
            msg="Unexpected for infinite impressions",
        )

    def test_by_spend(self):
        rp = ReachPoint(impressions=[10], kplus_reaches=[3], spends=[1.0])
        model = self.cls(data=[rp], ncomponents=2)
        res = model.by_spend(spends=[0.0])
        expected = 0
        self.assertEqual(
            res._kplus_reaches[0],
            expected,
            msg="Unexpected for zero spend",
        )
        res = model.by_spend(spends=[1.0])
        expected = 3
        self.assertEqual(
            res._kplus_reaches[0],
            expected,
            msg="Unexpected for same spend",
        )
        res = model.by_spend(spends=[2.0])
        expected = 3
        self.assertEqual(
            res._kplus_reaches[0],
            expected,
            msg="Unexpected for double spend",
        )
        ...
        self.assertEqual(
            res._kplus_reaches[0],
            expected,
            msg="Unexpected for infinite spend",
        )
