...
class HaloSimulatorTest(absltest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.params = SimulationParameters([0.04, 0.05], 12, 1e5)
    ...
    def test_simulated_reach_by_spend_no_privacy(self):
        reach_point = self.halo.simulated_reach_by_spend([0.04, 0.04], 100.0, 0.0, 3)
    ...
    def test_simulated_reach_by_spend_with_privacy(
        ...
        self.assertEqual(reach_point.reach(2), 4)
        self.assertEqual(reach_point.reach(3), 0)

    def test_privacy_tracker(self):
        self.assertEqual(self.halo.privacy_tracker.mechanisms, [])
        reach_point = self.halo.simulated_reach_by_spend([0.04, 0.04], 1.0, 0.0, 3)
        self.assertEqual(self.halo.privacy_tracker.mechanisms, ["Laplace"])
